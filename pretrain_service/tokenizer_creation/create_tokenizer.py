...
def initialize_tokenizer(args):
    ...
    tokenizer.decoder = decoders.Sequence([
        decoders.ByteLevel(add_prefix_space=True, use_regex=True),
        decoders.Strip(' ', 1, 0),
        decoders.Replace("\n ", "\n")
    ])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_path', type=Path, default="data_cleaned/babylm_10M/full_train.txt", help='Specify the input filename')
    parser.add_argument('--validation_path', type=Path, default="data_cleaned/babylm_10M/full_train.txt", help='Specify the validation filename')
    parser.add_argument('--vocab_path', type=Path, default="pretrain_service/tokenizers/tokenizer_1M.json", help='Specify the output filename')
    parser.add_argument('--output_dir', type=Path, default="pretrain_service/tokenizers/", help='Specify the output directory')
    ...
    print("Tokenizer creation completed successfully!", flush=True)
    
    # BabyLM-community/babylm-baseline-10m-gpt2
    print("Wrapping tokenizer with PreTrainedTokenizerFast", flush=True)
    ...
    for text in texts:
        tokens = fast_tokenizer(text)
        print(tokens)
        print(fast_tokenizer.decode(tokens["input_ids"]))

    # Save fast tokenizer
    print(f"Saving PreTrainedTokenizerFast to {args.output_dir}", flush=True)
    fast_tokenizer.save_pretrained(args.output_dir)

    print("Reloading PreTrainedTokenizerFast", flush=True)
    fast_tokenizer = PreTrainedTokenizerFast.from_pretrained(args.output_dir)
    print("Success!")
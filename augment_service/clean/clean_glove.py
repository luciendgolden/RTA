...
def clean_childes(lines):
    """ For aochildes, we lowercase and remove the space between the punctuation mark and the final word"""
    new_lines = []
    for line in lines:
...
def clean_gutenberg(lines):
    """ For gutenberg we extract narrative content, remove metadata, headings, and artifacts, and normalize text """
    paragraphs = []
    paragraph = ""
    skip_metadata = True
    in_footer = False
    ...
    for line in lines:
        ...
        tmp_line = line.lower().strip().replace('_', '').replace('’', "'")
...
if __name__ == "__main__":
    ...
    all_files = []
    concatenated_data = {folder: [] for folder in BABYLM_FOLDERS}
    ...
    for file in all_files:
        ...
        if folder:
            concatenated_data[folder].extend(tokenized_lines)

        # Write the new file to glove/ directory
        new_file = file.replace('original', 'glove')
        print(f"Saving to {new_file}")
        os.makedirs(os.path.dirname(new_file), exist_ok=True)
        with open(new_file, 'w', encoding='utf-8') as f:
            f.writelines(tokenized_lines)
            
    # write concatenated full.{train|dev|test} files for each folder
    for folder in BABYLM_FOLDERS:
        if concatenated_data[folder]:
            split = 'train' if folder in ['10M', '100M'] else folder
            output_dir = f"{BABYLM_ORIGINAL_DATA_PATH.replace('original', 'glove')}/{folder}"
            os.makedirs(output_dir, exist_ok=True)
            full_file_path = f"{output_dir}/full.{split}"
            print(full_file_path)
            with open(full_file_path, 'w') as f:
                f.writelines(concatenated_data[folder])